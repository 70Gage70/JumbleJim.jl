...
# ╔═╡ e7ab96b0-b8f1-11ef-310a-cf890cdbb284
begin
	using CSV, JLD2, DataFrames, CairoMakie
end

# ╔═╡ 1907128a-604d-48d4-a3f5-61fd3245d28a
begin
...
# ╔═╡ a74d502b-742d-4ef7-b82f-1a88de5c975d
"""
	punchline2spaces(punchline)
"""
function punchline2spaces(punchline)
	res = collect(uppercase(punchline)) .|> x -> x in collect('A':'Z') ? '_' : x
	return join(res)
end

# ╔═╡ c4e303f1-7256-4d69-8255-8bdc0d29e5dc
jumbles = jldopen("jumbles.jld2")["jumble"]
...
begin
	...
	setup = jumbles[JOKE_IDX, :setup]
	text!(ax, 0.05, 0.9, text = setup)
	
	punchline = jumbles[JOKE_IDX, :punchline] |> punchline2spaces |> collect
	for i = 1:length(punchline)
		c = punchline[i] |> x -> x == '_' ? "__" : string(x)
		text!(ax, 0.05 + 0.04*(i - 1), 0.7, text = c, font = :bold)
	end

	words = jumbles[JOKE_IDX, :words_jumbled]
	for w_idx = 1:length(words)
		...
		# spaces
		circles = jumbles[JOKE_IDX, :circles][w_idx]
		for i = 1:length(w)
			xmin = 0.15 + 0.04*i
...
let
	...
	for i = 1:length(punchline)
		...
		# solution
		pl = jumbles[JOKE_IDX, :punchline] |> uppercase
	...
	for w_idx = 1:length(words)
		...
		for i = 1:length(w)
			...
			word = jumbles[JOKE_IDX, :words][w_idx]
...
manifest_format = "2.0"
project_hash = "31133ad2b25a05d62920ff2043cba1c6f90583e5"
